...
| single physical reader | **частично** | читатель и так один: и одноразовая команда, и интерактивная сессия качают его по очереди, а одновременность запрещена контрактом. Постоянного фонового цикла пока нет — он появится вместе с экраном терминала |
...
| interactive PTY shell | **нет** | `AdbShellFrameBuffer` собирает рамки по мере поступления, `AdbInteractiveShell` держит живую сессию: шаг цикла обычной функцией, ввод рамками `stdin`, `Ctrl+C` байтом, конец ввода отдельной рамкой. Экрана и потока исполнения ещё нет — это следующий пункт. Критерий прогона — `07` §6.26 |
