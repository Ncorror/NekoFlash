...
Модулей семь: `:app`, `:core:model`, `:core:diagnostics`, `:core:operation`, `:usb:api`, `:usb:android`, `:protocol:adb`. Тестов 372 (`@Test` в текущем дереве).
...
Текущая работа — **Phase 4: ADB professional services**. AUTH blocker закрыт аппаратно; read-only Sync (`STAT`/`RECV`) доказан на малом и 2 MiB файле с совпавшим SHA-256 (`07` §6.29–§6.32). Полноценное сохранение pull-artifact, `SEND`, install, reboot, raw services и настоящий concurrent service dispatcher остаются открыты.
