...
| reboot | **протокольный путь есть, вызывающего в UI нет** | `AdbReboot` и `AdbConnection.reboot`, 12 тестов. Поведение открыто в Legacy `AdbServiceCompletionPolicy` перед кодом: сервис односторонний, устройство рвёт USB не ответив, и разрыв после целиком записанного `OPEN` — успех, а не отказ; послабление строго для `reboot:*`. Граница мутации — записанный `OPEN`, состояние устройства сообщается отдельно от исхода (`AdbRebootState`). Два расхождения с Legacy записаны с обоснованием: регистр произвольной цели не трогается, и молчание при живом транспорте не считается успехом. **Готовым не считается**: из UI перезагрузку не вызвать, и на железе не проверялось |
