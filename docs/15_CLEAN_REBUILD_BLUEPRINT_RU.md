...
| ktlint/detekt | нет | нет | нет (пока) |
...
Version catalog в новом репозитории уже есть — это сделано правильно. А вот статический анализ стиля и сложности (Kotlin official code style + ktlint, или detekt с правилами на сложность/размер и запрет нежелательных импортов между модулями) не появлялся ни в Legacy, ни в A2, ни пока в новом репозитории. Отсутствие именно такого гейта — одна из причин, по которой файлы на 2000–4000 строк выросли никем не остановленными. Стоит закрепить ADR-ом до того, как код разрастётся снова, а не после.
...
Этот Blueprint — справочник для планирования и код-ревью, дополняющий `docs/00`–`docs/14`; при расхождении побеждают канонические доки репозитория. Перед тем как Phase 2 продвинется дальше, имеет смысл закрепить ADR-ами четыре решения, которые сейчас существуют неявно: (1) `kotlinx.coroutines`/`Flow` как конкурентная модель; (2) подход к DI (ручной или библиотека — на масштабе 4 модулей ручной вполне достаточен, но выбор должен быть записан, а не подразумеваться); (3) ktlint/detekt как обязательный CI-гейт; (4) числовые пороги раздела 4.1 — как ориентир для ревью или как жёсткая policy, если после 2–3 итераций выяснится, что ориентир регулярно нарушается.
