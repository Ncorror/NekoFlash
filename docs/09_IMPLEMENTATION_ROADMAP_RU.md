...
| CNXN/AUTH | **частично** | подтверждены на железе: рукопожатие, авторизация с диалогом, повторное подключение по сохранённому ключу, отключение кабеля при подключённом ADB, перезапуск приложения с сохранением ключа (`07` §6.13, §6.14, §6.17). Не проверены: отказ в диалоге, Recovery, Sideload, хост ниже API 28 |
