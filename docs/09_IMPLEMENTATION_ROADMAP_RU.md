...
| concurrent service dispatcher | **работает, доказано; осталась одна проверка на тридцать секунд** | `AdbStreamRouter` умеет маршрутизировать несколько logical streams, но production ownership пока даёт один активный reader consumer: цикл `reader.read()` крутит сам потребитель, поэтому потребитель может быть только один, и `AdbLinkController.busy()` гасит кнопки при живой оболочке. Порядок перевода записан до кода в `docs/adr/0004_CONCURRENT_ADB_DISPATCHER_RU.md` (**PROPOSED**, D034). Шаг 1 выполнен: `AdbStreamMailbox` и `AdbStreamDispatcher`, 24 теста. Шаг 2 выполнен: `AdbServiceCall` берёт вывод из своего ящика, а не из событий маршрутизатора; пакеты в диспетчер пока подаёт его же поток, поэтому одновременности ещё нет и заявлять её нечем. Шаг 3 выполнен: то же самое для `AdbSyncExchange` — сессия `sync:` держит ящик на всё время жизни, а конец потока отдаётся ящиком сколько угодно раз, поэтому после обрыва операции отказывают той же причиной, а не ждут байт, которых не будет. Шаг 4 выполнен: `AdbInteractiveShell` тоже читает свой ящик, и изоляция от соседнего потока держится теперь ящиком, а не сравнением идентификаторов. Соединение больше не держит маршрутизатор отдельно: напрямую к нему не обращается никто. Доказательство сохранности поведения на всех трёх шагах — прежние поведенческие сюиты `AdbServiceCallTest`, `AdbSyncSessionTest`, `AdbSyncSendTest`, `AdbInteractiveShellTest` и `AdbConcurrentStreamsTest` прошли без правок, кроме одной строки харнесса в каждой. Диспетчер остался **без ввода-вывода и без потока исполнения**, как `AdbStreamRouter`: цикл и его scope переходят к `AdbConnection` шагом 2, поэтому раскладка проверяется тестами без единого потока. Ожидание на ящике проверено настоящим потоком и сразу нашло дефект: одной записи поля `terminal` мало, уснувший потребитель ею не будится и досиживал до таймаута — конец теперь кладётся и в очередь. **Готовым пункт не считается**: диспетчер пока не вызывает никто, а `docs/04` capability completeness rule засчитывает только работающий production-путь. Порядок шагов исправлен после шага 1: цикл переезжает в `AdbConnection` не первым, а предпоследним, потому что работающий цикл рядом с непереведённым потребителем дал бы двух физических читателей — разбор в ADR. Шаги 5 и 6 выполнены: читающий цикл `AdbDispatchLoop` переехал в `AdbConnection` и стал один на соединение, `busy()` снят, экран больше не гасит кнопки при живой оболочке, операции идут на пуле потоков вместо одной очереди. `AdbPacketWriter` сериализует отправку сам — с появлением цикла у записи стало три источника, и требовать сериализации от владельца стало нельзя. Доказано двумя прогонами. `07` §6.40: оболочка держала поток девять минут, внутри окна прошли три файловые операции по другим потокам, отпечаток совпал с §6.29 — до изменения ни одна из них не запустилась бы. `07` §6.41: одноразовые команды прошли при живой оболочке, вывод пришёл в свой слот, а при обрыве кабеля оболочка и чтение файла закрылись **одной** причиной. Там же ящик оболочки переполнился на `logcat` — поведение заложенное (закрыть поток, а не потерять вывод), соседей и соединение не тронуло. `07` §6.42 закрыл `Ctrl+C`: прерывание названо по номеру потока, соседний поток через пятнадцать секунд ответил, оболочка его пережила. Там же трёхминутный `logcat` ящик не переполнил — 181 с против 95 с до развязки разбора с показом на экране, то есть гипотеза §6.41 подтверждена, и объём ящика менять незачем. **Готовым пункт пока не считается** из-за одной невыполненной части строки 6: файловая операция во время `logcat` не выполнялась ни в одном прогоне. Засчитывать её «по существу» нельзя — правило чтения гейта говорит «все строки», а менять критерий после результата запрещено. До этой проверки ADR остаётся `PROPOSED`. Это архитектурный долг, а не искусственный product gate |
