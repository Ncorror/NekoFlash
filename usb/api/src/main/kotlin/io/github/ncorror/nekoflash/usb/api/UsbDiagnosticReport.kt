 ...
 *
 * Этот отчёт — **raw evidence**: serial/TargetId/connection metadata сохраняются
 * для корреляции аппаратных прогонов. Это не означает, что privacy считается
 * capability restriction: при экспорте оператор предупреждается о содержимом,
 * а архив получает отдельный `privacy.txt` с `sanitized=false`.
 *
