...
| Режим peer'а | **частично** | режим читается из баннера `CNXN`; на устройстве подтверждены `DEVICE` и `RECOVERY` (`07` §6.24). Sideload не проверен. Различение `fastboot` и `fastbootd` здесь невозможно в принципе: в Legacy оно бралось из `getvar is-userspace`, то есть после протокольного обмена, и придёт вместе с движком Fastboot |
...
| single physical reader | **ждёт железа** | цикл живёт на исполнителе с единственным потоком и занимает его целиком, пока сессия жива. Одноразовые команды в это время отклоняются — не откладываются, а именно отклоняются, потому что читатель один. Ввод идёт из другого потока: приём и передача по разным эндпоинтам, общее состояние под замком, ожидание пакета снаружи замка |
...
| legacy fallback | **частично** | откат на `shell:` написан и покрыт тестами: срабатывает и когда устройство не объявило `shell_v2`, и когда объявило, но поток не открылся. На железе не проверялся — нужен peer без `shell_v2`, например Recovery |
| interactive PTY shell | **ждёт железа** | `AdbShellFrameBuffer` собирает рамки по мере поступления, `AdbInteractiveShell` держит живую сессию: шаг цикла обычной функцией, ввод рамками `stdin`, `Ctrl+C` байтом, конец ввода отдельной рамкой. На экране подключённого устройства есть терминал: ввод, `Ctrl+C`, закрытие. Читающий цикл живёт на выделенном потоке. Критерий прогона — `07` §6.26 |
| real terminal UI | **ждёт железа** | вывод, поле ввода, `Ctrl+C` и закрытие на экране подключённого устройства. Управляющие последовательности пока показываются как есть — эмулятора терминала нет, и на экране об этом сказано |
