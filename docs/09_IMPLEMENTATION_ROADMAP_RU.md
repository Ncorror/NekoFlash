...
| one transaction engine | **написано, гейт не пройден** | `FastbootLane` и есть этот движок, и второго пути нет: опрос роли, произвольная команда, чтение переменной и `getvar:all` идут через него одного. Три обмена разной формы — это и есть то, чем пункт закрывается, потому что «движок один» подтверждается не существованием класса, а тем, что через него прошло непохожее. Гейт `07` §6.71 записан до прогона и ждёт его |
| INFO/TEXT/OKAY/FAIL | **написано, гейт не пройден** | Все четыре типа разбираются и различаются по существу: `INFO` и `TEXT` обмен не кончают и копятся, `OKAY` и `FAIL` кончают, причём `FAIL` — обычный ответ устройства, а не поломка, и рамку он не портит. Непонятый кадр сохраняется целиком. Теперь это видно и оператору: секция консоли показывает строки `INFO` отдельными строками, отказ — словом устройства, и состояние полосы рядом. Отказ и согласие различаются словом, а не местом на экране: прятать `FAIL` в другую секцию значило бы подменять ответ оценкой. Гейт `07` §6.71 ждёт прогона |
| raw command | **написано, гейт не пройден** | Поле свободное, списка команд нет (`01` §3): что бывает у Fastboot, знает устройство. `FastbootLinkController.runCommand` шлёт набранное как набрано и показывает ответ вместе с состоянием полосы. Две ветки провода теперь вызываются с экрана и названы, а не подменяются: команда не в ASCII и команда длиннее кадра (ADR-0006 §4). Гейт `07` §6.71 ждёт прогона |
| getvar explorer | **написано, гейт не пройден** | Поле произвольного имени и кнопка «Прочитать всё». Разбор `getvar:all` написан по обоим архивам, и они совпали (`FastbootGetVarAllParser.kt` в Legacy и A2): строки несут префиксы `INFO`, `TEXT` и **`(bootloader)`**, снимать их надо в цикле; последнее значение главное, как в `fastboot` CLI, но все расхождения под одним именем сохраняются; имя с разделом содержит **второе** двоеточие, и наивное разрезание по первому слило бы все разделы в одну переменную. Неразобранная строка сохраняется, а не выбрасывается: выбросить её значило бы выдать неполный разбор за полный. Сборка разделов в инвентарь намеренно не делается — это Phase 6. Гейт `07` §6.71 ждёт прогона |
