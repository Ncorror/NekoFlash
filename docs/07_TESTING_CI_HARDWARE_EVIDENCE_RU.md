...
Состав отчёта ничем не урезан. Он собирается оператором о собственном устройстве, и вырезать из него сведения «на всякий случай» означало бы ровно тот host-side запрет, который запрещает `01_PRODUCT_CHARTER_RU.md`. Поле, которого платформа не даёт, записывается с причиной отказа, а не пропускается: `Build.getSerial()` без `READ_PHONE_STATE` отвечает отказом, и это отказ платформы, который по классу Device authority показывается как есть.
