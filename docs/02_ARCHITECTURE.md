...
The original USB evidence probe still sends no protocol bytes. The bounded ADB probe sends only one `CNXN` plus the minimum `AUTH` response sequence required by the peer; the same probe is invoked automatically by the auto-first coordinator for one unambiguous canonical ADB target and remains available as an explicit diagnostic/retry action. It never sends `OPEN`, `WRTE`, shell, sync/push, reboot or Fastboot bytes. It classifies a claim candidate only by the concrete transport need for both bulk IN and bulk OUT; vendor, product, class, subclass and protocol are recorded as evidence rather than used as Xiaomi/Poco bans.
