...
| reboot | **готово** | `AdbReboot` и `AdbConnection.reboot`, 12 тестов. Поведение открыто в Legacy `AdbServiceCompletionPolicy` перед кодом: сервис односторонний, устройство рвёт USB не ответив, и разрыв после целиком записанного `OPEN` — успех, а не отказ; послабление строго для `reboot:*`. Граница мутации — записанный `OPEN`, состояние устройства сообщается отдельно от исхода (`AdbRebootState`). Два расхождения с Legacy записаны с обоснованием: регистр произвольной цели не трогается, и молчание при живом транспорте не считается успехом. Вызывающий в production есть: `AdbRebootController` и своя секция на экране со свободным полем цели. **Готовым не считается**: на железе не проверялось. Доказано на железе прогоном `07` §6.46: пустая цель, `bootloader` (generation поднялась с интерфейсом **FASTBOOT** — смена роли это наблюдение, а не толкование), `recovery` и произвольная строка. Прогон нашёл дефект: ветка ответа текстом стояла после границы мутации, а отвечала `UNTOUCHED`; recovery сказала `reboot.` и ушла в перезагрузку через 336 мс. Причина переименована в `DEVICE_ANSWERED`, состояние — `UNKNOWN`; слово устройства передаётся дословно и не переводится в вердикт. Дальше: после ответа словами решает **шина**, а не текст — если устройство уходит, это переход, и слова переносятся в evidence. Гейт на это записан до кода (`07` §6.47) и прогонялся дважды. §6.48 показал, что улучшение не работало вовсе: судьбу транспорта спрашивали у цикла раскладки, а он о чужой беде не знает; спрашивать надо у самого интерфейса (`AdbTransportEnd.of`). §6.50 на исправленной сборке: все пять шагов гейта пройдены, `reboot_failed` за прогон нет, обе дороги к `accepted` (конец от читателя и остановка цикла по просьбе) встретились и обе верны. Ветка ответа словами **на железе так и не вызвана** — устройство ни разу не сказало ни слова, и подтверждать пока нечего. Входит она через раз на комбинации «из recovery, цель `bootloader`», поэтому закрывается повторением шага, а не новым сценарием |
...
| forward/reverse | **частично / `forward` написан, на железе не проверен** | Условие «если current protocol layer готов» выполнено ADR-0004. Замысел и порядок — `docs/adr/0005_LOCAL_SOCKET_FORWARDING_RU.md` (**PROPOSED**, D037). Архивы проверены до кода: ни в Legacy, ни в A2 нет ни `forward`, ни `reverse`, ни одного сокета, поэтому проектируется с нуля, как требует `16` §3. `forward` и `reverse` — разные механизмы: первому устройство не сообщает ничего (`tcp:5555` это обычный сервис ADB, реестр живёт на хосте), второй слушает устройство и присылает входящий `OPEN`, которого маршрутизатор не принимает вовсе. Поэтому пункт делится. Готово по `forward`: `AdbForwardStream` в протокольном модуле (16 тестов, сокетов не знает), `AdbForwardController` со слушателем на loopback (14 тестов), секция на экране с двумя свободными полями, `INTERNET` в манифесте как требование платформы к `AF_INET`. **Готовым не считается**: гейт `07` §6.51 прогнан и встал на шаге 1 — слушатель не заводится вовсе, `socket failed: ECONNREFUSED` **на создании** сокета, до всякой привязки, одинаково для порта 0 и 5555 (`07` §6.52). Чей это отказ, тот прогон различить не позволил: состава разрешений в отчёте не было. Добавлены `app.permissions` в отчёт и состояние разрешения прямо в строку отказа, и следующий прогон закрыл вопрос одной строкой (`07` §6.53): `INTERNET=granted`, а сокет всё равно не создаётся. Причина найдена и подтверждена действием (`07` §6.54): установка **поверх** сборки без `INTERNET` не даёт хосту выдать сетевой доступ; после чистой установки слушатель поднялся с первой попытки. Шаг 1 гейта пройден — слушатели заводятся надёжно, четыре подряд, включая адрес кириллицей (`01` §3 соблюдён наблюдением). Шаги 2–7 остаются: соединений не было ни одного, и прогон вскрыл дефект наблюдаемости — «ноль обслужено» не отличалось от «клиент не приходил», а снятие проброса не писалось в журнал вовсе (`07` §6.55). Добавлены `forward_accepted`, `forward_removed`, `forward_stopped` и счётчик принятых на экране — именно они позволили не гадать дальше. Причина найдена и подтверждена контролируемым изменением (`07` §6.57, §6.58): `getLoopbackAddress()` отдавал не тот loopback, на который стучится клиент. После явной привязки к `127.0.0.1` гейт §6.51 закрыт целиком прогоном `07` §6.59: 18 соединений, `718/718` байт в обе стороны, **четыре** одновременных потока вместо требовавшихся двух, `CLIENT_CLOSED` и `REJECTED` как разные причины, снятие с освобождением порта и снятие по смерти транспорта. **`forward` доказан на железе целиком.** Пункт остаётся «частично» только из-за `reverse`. Шаг 6 плана сделан: маршрутизатор разбирает входящий `OPEN` от устройства и отдаёт его событием, **не подтверждая сам** — подтвердить и передумать значило бы сказать «принял», не приняв, ведь на `OKAY` устройство вправе сразу слать данные. Судьбу такого потока решают `acceptInbound`/`rejectInbound`; диспетчер пока корректно отказывает, потому что принимать некому, и это правильный ответ, а не заглушка. Остаются шаги 5 и 7: формат запроса выясняется наблюдением через поле произвольного сервиса, и сам `reverse` поверх него. `reverse` не начат: входящий `OPEN` и формат запроса, который выясняется наблюдением, а не по памяти |
