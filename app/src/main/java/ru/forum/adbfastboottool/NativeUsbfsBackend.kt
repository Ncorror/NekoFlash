...
/**
 * Production native USBFS DATA sender for Fastboot payloads.
 *
 * The backend has a single-transfer gate, tokenized cancellation, separate stall
 * and hard deadlines, strict DISCARDURB -> REAP-before-free semantics and explicit
 * poisoned-state reporting so native buffer lifetime remains valid while usbfs owns URBs.
 */
                    ...
                    while (polling.get()) {
                        ...
                        if (state.nativeTransferActive && state.nativeTransferToken == token) {
                            ...
                            try {
                                ...
                                // Progress reporting is observational and must not abort transport.
                            }
                        }
                        try {
        ...
        if (progressThreadStopped) {
            onProgress?.let { callback ->
                try {
                    ...
                    // Final progress is best-effort and never changes transfer outcome.
                }
            }
        }
