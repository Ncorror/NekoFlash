...
## Второй Android smoke test alpha5

Build `6.0.0-alpha5-dev-nekoflash+0747c4ec72e3.29866798716` был проверен без USB-устройства.

Подтверждено:

- Fastboot DATA main/advanced taps оставляют точный compact-log отказ без запуска transport operation;
- импорт и анализ ZIP работают без ADB/Fastboot session;
- WebView Mi Account вход доходит до успешного user ID `4159543393`.

Обнаружено:

- после успешного WebView login background token exchange ошибочно блокировал официальный `https://unlock.update.miui.com/sts` как account-only URL;
- статическая зелёная галочка у Sideload checksum note выглядела как уже выполненная проверка.

Focused fix разделяет интерактивный account allowlist и bounded regional Mi Unlock `/sts` allowlist. Реальный повторный login/token exchange остаётся обязательным; никаких account tokens в sanitised evidence не сохраняется.
