...
## Hardware observations carried into Phase 2 ADB handshake work
...
- POCO F5 (`marble`) has Android-mode runs where the host `UsbManager` reports `deviceCount=0`; the owner also confirms the same physical F5 is observable in Fastboot. The Android-mode enumeration quirk is therefore tracked below the ADB layer and is not used to block ADB handshake work on an enumerating target.
- The exact `NekoFlash-main.zip` reference contains real POCO F5 Fastboot evidence: `vendorId=0x18D1`, `productId=0xD00D`, `interfaceKind=FASTBOOT`, `getvar:product -> marble`, `getvar:unlocked -> yes`, and a complete `getvar:all` snapshot with 290 variables before later bootloader/fastbootd read experiments.
- The A2 reference contains a separate `marble` recovery observation: host-side refresh could rediscover and claim the Fastboot interface after a failure, but `getvar:product` writes still returned `sent=-1`; leaving and re-entering Fastboot on the target caused a real detach/attach re-enumeration, after which writes and a new read-only operation succeeded. This is device-specific hardware evidence, not a universal retry/reboot policy.
- The intermittent ADB root cause remains `UNKNOWN`. These observations justify descriptor/permission/claim/bulk-transfer/CNXN/AUTH tracing, not a host-side Xiaomi/Poco/HyperOS capability ban. Device/peer protocol behavior remains authoritative.
...
Current bundle sections are `summary.txt`, `usb-events.txt`, `adb-events.txt`, `usb-descriptors.txt`, `device-info.txt`, `app-build.txt`, and `session-info.txt`, with `export-manifest.txt` first. `adb-events.txt` is intentionally empty until the explicit handshake probe is run. This format is deliberately extensible: later ADB, Fastboot and Native USBFS evidence can add separate named files while keeping the current USB evidence readable and grep-friendly.

Commit `cf7da5fa7d9357600b778bb76fa9f6f178cf8e98` has authoritative verification for the multi-file bundle state: 14/14 JVM tests PASS, USB lint reports no issues, and app lint reports 0 errors with 11 non-blocking warnings. The owner then produced four real archives on a two-device session involving POCO X3 Pro and POCO F5: `NekoFlash-evidence-20260917-215229Z.zip`, `...215248Z.zip`, `...215508Z.zip`, and `...215550Z.zip`. All four opened successfully and contained the manifest plus all six expected files, so the ZIP save path itself is hardware-validated.

Those archives also exposed an evidence semantics defect that must be fixed before ADB work. All four carried the same process-wide `runId`, and later archives accumulated earlier USB events. The first pair contained the known vayu-like `18D1:4EE7` / `FF/42/01` / bulk `0x01`+`0x81` / MPS 512 descriptor shape, while the later pair recorded fresh scans with `deviceCount=0`; because the bundle did not record an owner target label, the archive format alone cannot prove which physical phone produced each pair. In addition, a `permission_granted` event could coexist with `summary.txt` reporting `permissionGranted=false` because the summary serialized the last Compose-held device snapshot from before the asynchronous permission callback.

The follow-up therefore introduces an explicit per-target evidence session without replacing the Application-scoped USB receiver: **New evidence session** clears only the in-memory sink, rotates `sessionId`, and resets the visible target state. The owner supplies a `targetLabel` such as `POCO X3 Pro` or `POCO F5`, and both TXT/ZIP export perform a fresh `UsbManager` scan immediately before serialization. `summary.txt` and `session-info.txt` move to v2 semantics with `sessionId` and `targetLabel`. Commit `23c82b1028ae0cda23a68e8bd31b0497dff0ef72` has authoritative CI verification for this session fix: 15/15 JVM tests PASS, USB lint 0 issues, app lint 0 errors / 11 warnings. Fresh two-device owner validation remains useful, but the owner elected not to block the next ADB evidence slice on the rare F5 Android enumeration case.


## Phase 2 minimal ADB CNXN/AUTH slice — pre-CI

The next changeset adds a narrow executable `:protocol:adb` module plus an Android USB adapter. Its only wire behavior is one `CNXN` followed, when requested by the peer, by `AUTH SIGNATURE` and then `AUTH RSAPUBLICKEY` after a repeated token. It does not contain ADB `OPEN`/`WRTE`, shell, sync/push, reboot, package management, recovery services, or Fastboot commands.

The Android adapter requires the protocol interface shape `FF/42/01` and a bulk IN/OUT pair but does not whitelist any VID/PID. It uses `openDevice` + `claimInterface(false)`, performs no automatic reconnect/second-CNXN/endpoint-halt clear, and releases/closes after the terminal handshake result. Each USB call records requested bytes, returned bytes, call duration and timeout budget. Packet evidence records only command/type names and payload lengths. AUTH tokens, RSA signatures, public-key material, private-key paths and raw peer banners are excluded by construction.

The host RSA key is persistent in app-private storage so user authorization can survive later runs, but no key material is exported. The evidence ZIP gains `adb-events.txt`, while USB descriptors/events remain separate. This section is implementation/pre-CI evidence only; no new ADB hardware PASS is claimed until the owner runs the APK on an enumerating target and supplies the resulting bundle.
## Phase 2 POCO X3 Pro ADB handshake hardware PASS — 2026-09-18

Owner archive `NekoFlash-evidence-20260917-231324Z.zip` is the first rewrite hardware proof beyond open/claim for the bounded ADB slice. The bundle identifies `targetLabel=poco x3pro`, one enumerated device `18D1:4EE7`, permission granted, canonical ADB interface index `0` (`FF/42/01`), bulk OUT `0x01`, bulk IN `0x81`, and max packet size 512. `openDevice` succeeded and `claimInterface(false)` returned true.

The recorded wire sequence was `CNXN -> AUTH TOKEN -> AUTH SIGNATURE -> AUTH TOKEN -> AUTH RSAPUBLICKEY -> CNXN`. The target-side RSA approval completed during the already-running probe. Terminal evidence was `handshake_finished outcome=CONNECTED authPath=PUBLIC_KEY peerKind=DEVICE`, followed by successful interface release and connection close. Total probe elapsed time was approximately **1801 ms**. AUTH tokens, signature bytes, public-key bytes and the raw peer banner were not exported.

This proves the rewrite's USB + ADB `CNXN`/`AUTH` boundary on that POCO X3 Pro run. It does **not** prove ADB services (`OPEN`/`WRTE`), shell, sync/push, reboot, recovery services, Fastboot, or the root cause of historically intermittent Poco/Xiaomi attachment behavior. The owner explicitly chose to continue with product-flow work rather than block on the POCO F5 Android-mode enumeration quirk because F5 is observable in Fastboot and X3 Pro normally works with only rare connection delay.

A separate carried-forward product observation is now protected by ADR 0004: Legacy and A2 made ordinary USB attach/startup progression automatic. The rewrite must preserve that usability while keeping manual controls available for diagnostics and ambiguity.

## Phase 2 auto-first verification and full capability audit — 2026-09-18

Commit `54b89ccf3f255dd8dc0fa4614a4a393f74211eda` has authoritative verification for the restored auto-first USB/ADB entry implementation: 31/31 JVM tests PASS, 0 failures/errors/skips, `:transport:usb-android` lint reports no issues, and app lint reports 0 errors with 11 non-blocking warnings. This proves the code/test/lint gate for startup/attach orchestration; owner hardware acceptance without pressing the diagnostic sequencing buttons remains open and is tracked only in `03_ROADMAP.md`.

The full cross-archive/current-protocol audit is recorded in `06_FULL_PROTOCOL_AUDIT.md`. It compares Legacy, frozen A2, previous `main` and the rewrite instead of treating any one archive as a complete feature oracle. Confirmed gaps include ADB Sync `LIST` (documented but not implemented in previous `main`), modern Sync v2/compression, modern Wireless ADB pairing/TLS/mDNS, explicit/verified host-to-device ADB USB Zero-Length Packet (ZLP) semantics at endpoint max-packet boundaries, incremental/atomic multi-package install workflows, ADB Burst Mode, Fastboot factory `update`/`flashall`, Fastboot staging/network transports, and complete end-to-end sparse flashing. The audit also distinguishes raw wire-command presence from a complete desktop-style host workflow (for example `wipe-super`). These are audit findings, not new hardware PASS claims.

The same audit reaffirmed the professional-tool product contract in ADR `0005_PROFESSIONAL_CAPABILITY_POLICY.md`: correctness/platform invariants remain strict, but NekoFlash does not create Novice/Expert permission profiles, hidden capability tiers, product command allowlists/denylists, vendor whitelists, or other host-side authorization that pre-empts a valid peer/device request. This is a product/architecture rule, not a hardware capability claim.
