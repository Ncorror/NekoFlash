...
## D034 — Читающий цикл принадлежит соединению, а не потребителю (PROPOSED)
...
Статус **PROPOSED**: решение переписывает механизм, через который проходит каждый аппаратно доказанный путь Phase 3, и подтверждается прогоном на устройстве, а не сборкой. Замысел, порядок переноса потребителей и список того, что обязан доказать прогон, — `docs/adr/0004_CONCURRENT_ADB_DISPATCHER_RU.md`.
