...
|---|---|---|---|
| Файлов kt/java | 44 | 96 | 107 |
| Строк кода всего | 17 274 | 18 283 | 22 217 |
| из них тестов | **0** | 4 399 (~24%, 7 файлов) | 9 685 (~44%, 41 файл) |
| Крупнейший файл | `MainActivity.kt` — 3880 строк | `UsbSessionCoordinator.kt` — 2495 строк | `AdbWorkspace.kt` — 701 строка |
...
Колонка «сейчас» — снимок на **2026-09-11**, и она устаревает при каждом
changeset. Сверять её надо, а не доверять: `find . -name '*.kt' | xargs wc -l`.
Аргумент таблицы от чисел не зависит и с ростом кода только усиливается: Legacy
дал 17 274 строки при нуле тестов, A2 довёл долю тестов до 24 %, новая база —
до 44 %, а её крупнейший файл в пять с половиной раз меньше крупнейшего файла
Legacy. Прошлый снимок называл соотношение «в восемь раз» и к аудиту перед
Phase 5 разошёлся с деревом: цифра, выведенная из снимка, устаревает вместе с
ним, а звучит как постоянная.
...
**Где `:app` сегодня отступает от этой строки — и почему это записано, а не
скрыто.** В `:app` лежат `AdbLinkController`, `AdbSyncController` и
`AdbTerminalController` — оркестрация, а не «граф и навигация». По карте
`docs/02` §12 им место в `feature:*`, которых пока нет.

Отступление сознательное. D025 запрещает заводить модули заранее, а у трёх
контроллеров сейчас один потребитель и одна навигация: ownership boundary,
ради которой создаётся модуль, ещё не появилась. Пока контроллеры остаются
тонкими и протокольной логики не содержат — она вся в `protocol:adb`, — цена
отступления нулевая.

Триггер на вынос: появление второго `feature:*`-потребителя тех же контроллеров
или первый контроллер, который начнёт знать про протокол больше, чем «вызвать и
показать исход». `:app` — единственный модуль без машинного гейта границ, и это
не упущение: он вправе импортировать всё. Поэтому здесь единственная защита —
ревью, и потому строка написана.
