...
/**
 * Извлекает динамическую палитру из bg_welcome image через Android Palette API
...
object ThemePalette {
    ...
    // Увеличивайте при смене bg_welcome image — старый кеш палитры сбросится.
    ...
    /**
     * Декодирует bg_welcome image и извлекает палитру. Тяжёлая операция —
    ...
    fun extractAndCache(context: Context): Scheme {
        return try {
            ...
            val bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.bg_welcome, opts)
                ?: return DEFAULT
