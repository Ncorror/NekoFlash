...
| Пункт | Статус | Где |
|---|---|---|
| bootloader/fastbootd classification | **написано, гейт не пройден** | Модуль `:protocol:fastboot`: `FastbootPacketCodec` (разбор кадра), `FastbootLane` (единственная синхронная полоса), `FastbootGetVar`, `FastbootModeProbe` — 52 теста. В production: `FastbootLinkController` и своя секция на экране, 5 тестов. Архивы открыты до кода (`16` §3): типы ответа и липкое состояние потери рамки сверены по обоим деревьям и совпали, признак роли `is-userspace` взят из Legacy `FastbootProtocol.logDiagnostics`, а доставка значения `getvar` двумя способами — из `readGetVarResponse`, по памяти этого не восстановить. Решение — `docs/adr/0006_FASTBOOT_SINGLE_LANE_AND_THREE_STATE_ROLE_RU.md` (**PROPOSED**, D038). Роль имеет **три** состояния, и `UNKNOWN` не сворачивается в «загрузчик» ни при каких условиях: в `fastbootd` работают операции с динамическими разделами, которых в загрузчике нет, и догадка выйдет боком ровно тогда, когда на неё обопрутся. Аппаратный гейт `07` §6.69 записан **до** кода и ещё не прогнан — на устройстве по Fastboot не выполнялось ни одной команды, весь разбор пока по архивам |
| one transaction engine | **частично** | `FastbootLane` и есть этот движок, и второго пути нет: и опрос, и всё, что появится дальше, идут через него. Готовым пункт станет, когда через него пройдут разные по форме обмены, а не один `getvar` |
| INFO/TEXT/OKAY/FAIL | **частично** | Все четыре типа разбираются и различаются по существу: `INFO` и `TEXT` обмен не кончают и копятся, `OKAY` и `FAIL` кончают, причём `FAIL` — обычный ответ устройства, а не поломка, и рамку он не портит. Непонятый кадр сохраняется целиком. Готовым пункт станет, когда это будет видно оператору на экране, а не только в структуре исхода |
| raw command | **нет** | Полоса принимает любую команду, но поля для неё на экране нет. Здесь же закроются ветки слишком длинной команды и команды не в ASCII — сейчас их нечем вызвать с устройства |
| getvar explorer | **нет** | `FastbootGetVar` умеет спросить любое имя, но в production вызывается с одним — `is-userspace`. Поля для произвольного имени и `getvar:all` нет |
