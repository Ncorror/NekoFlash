...
Захват интерфейса — исключительное владение ресурсом, а не право пользователя. Освобождение возвращает сессию из `CLAIMED` в `READY`, не завершая её: устройство остаётся подключённым, разрешение выданным, generation той же. Терминальным остаётся только `CLOSED`, и правило непригодности handle прошлой generation этим не ослабляется — handle становится непригоден при освобождении, независимо от состояния сессии.
