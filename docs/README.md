...
- `0005_LOCAL_SOCKET_FORWARDING_RU.md` — **ACCEPTED по `forward`** 2026-09-11 (`07` §6.59), `reverse` не начат: проброс портов. `forward` строится на существующем диспетчере и делается первым; `reverse` требует входящего `OPEN`, которого маршрутизатор не принимает, и формат его запроса выясняется наблюдением, а не по памяти.
