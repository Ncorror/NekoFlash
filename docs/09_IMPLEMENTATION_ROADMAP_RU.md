...
| reboot | **код готов, ждёт железа** | `AdbReboot` и `AdbConnection.reboot`, 12 тестов. Поведение открыто в Legacy `AdbServiceCompletionPolicy` перед кодом: сервис односторонний, устройство рвёт USB не ответив, и разрыв после целиком записанного `OPEN` — успех, а не отказ; послабление строго для `reboot:*`. Граница мутации — записанный `OPEN`, состояние устройства сообщается отдельно от исхода (`AdbRebootState`). Два расхождения с Legacy записаны с обоснованием: регистр произвольной цели не трогается, и молчание при живом транспорте не считается успехом. Вызывающий в production есть: `AdbRebootController` и своя секция на экране со свободным полем цели. **Готовым не считается**: на железе не проверялось. Гейт записан до прогона — `07` §6.44, и главная его строка не «перезагрузилось», а «отличили ли отказ от успеха»: и то и другое кончается потерей соединения |
| raw services | **код готов, ждёт железа** | `AdbRawServiceController` и своя секция на экране: имя сервиса вводится целиком и передаётся как набрано, списка нет (`01` §3). Размер ответа называется всегда и отдельно от текста — сервис может отвечать не текстом, и тогда число байт единственное достоверное. Односторонний сервис узнаётся **по имени**, как в Legacy: набранный здесь `reboot:` ведёт себя как кнопка перезагрузки, иначе ожидаемый разрыв показался бы отказом. Гейт до прогона — `07` §6.45 |
