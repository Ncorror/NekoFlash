...
| concurrent service dispatcher | **шаги 1–2 из 6, один потребитель переведён** | `AdbStreamRouter` умеет маршрутизировать несколько logical streams, но production ownership пока даёт один активный reader consumer: цикл `reader.read()` крутит сам потребитель, поэтому потребитель может быть только один, и `AdbLinkController.busy()` гасит кнопки при живой оболочке. Порядок перевода записан до кода в `docs/adr/0004_CONCURRENT_ADB_DISPATCHER_RU.md` (**PROPOSED**, D034). Шаг 1 выполнен: `AdbStreamMailbox` и `AdbStreamDispatcher`, 24 теста. Шаг 2 выполнен: `AdbServiceCall` берёт вывод из своего ящика, а не из событий маршрутизатора; пакеты в диспетчер пока подаёт его же поток, поэтому одновременности ещё нет и заявлять её нечем. Доказательство сохранности поведения — вся прежняя поведенческая сюита `AdbServiceCallTest` прошла без правок, кроме одной строки харнесса. Диспетчер остался **без ввода-вывода и без потока исполнения**, как `AdbStreamRouter`: цикл и его scope переходят к `AdbConnection` шагом 2, поэтому раскладка проверяется тестами без единого потока. Ожидание на ящике проверено настоящим потоком и сразу нашло дефект: одной записи поля `terminal` мало, уснувший потребитель ею не будится и досиживал до таймаута — конец теперь кладётся и в очередь. **Готовым пункт не считается**: диспетчер пока не вызывает никто, а `docs/04` capability completeness rule засчитывает только работающий production-путь. Порядок шагов исправлен после шага 1: цикл переезжает в `AdbConnection` не первым, а предпоследним, потому что работающий цикл рядом с непереведённым потребителем дал бы двух физических читателей — разбор в ADR. Шаги 3–6 (перевод `AdbSyncExchange`, `AdbInteractiveShell`, переезд цикла, снятие `busy()`) и аппаратный прогон впереди — правильность одновременных потоков не проверяется ни сборкой, ни подставным транспортом. Это архитектурный долг, а не искусственный product gate |
