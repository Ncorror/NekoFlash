 ...
 *
 * Это **raw evidence**, а не обезличенный support bundle: идентификаторы хоста
 * сохраняются намеренно, чтобы несколько аппаратных прогонов можно было
 * коррелировать. Privacy и capability policy — разные оси; поэтому UI и сам ZIP
 * явно предупреждают о чувствительных полях, а будущий sanitized export может
 * существовать рядом, не меняя raw-evidence формат.
 *
