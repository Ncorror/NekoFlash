...
object UsbDeviceInspector {
    ...
    enum class MatchKind(val rank: Int, val label: String) {
        CANONICAL(0, "canonical"),
        COMPAT_FASTBOOT(1, "compat"),
        GENERIC_FASTBOOT(2, "generic")
    }
    ...
    /** Выбирает лучший обнаруженный ADB/Fastboot bulk-интерфейс; generic-кандидат проверит handshake протокола. */
    fun selectPrimaryCandidate(device: UsbDevice, allowGenericFastboot: Boolean = true): Candidate? {
    ...
    /** Все обнаруженные кандидаты; окончательную пригодность подтверждает handshake ADB/Fastboot. */
    fun findAutoConnectCandidates(devices: Collection<UsbDevice>): List<Candidate> =
    ...
    fun selectModeSwitchCandidate(
        ...
        val changed = findAutoConnectCandidates(devices)
