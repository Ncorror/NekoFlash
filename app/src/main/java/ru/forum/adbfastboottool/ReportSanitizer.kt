...
object ReportSanitizer {
    ...
    const val REDACTED_DEVICE_ID = "<redacted:device-id>"
    ...
    private val miLoginIdRegex = Regex("(?i)(Signed in \\(ID:\\s*)[^)]+")
    private val miAuthorizedIdRegex = Regex("(?i)(Authorized\\.\\s*ID:\\s*)[^\\s,)]+")
    ...
    private val russianDeviceTextRegex = Regex("(?i)(Device:\\s*)([^|\\n\"]+)")
    ...
    private val sensitivePropertyBracketRegex = Regex(
        "(?i)(\\[(?:[^\\]]*\\.)?(?:imei\\d*|meid|imsi|iccid|android[_-]?id|cpuid|ufsid|fp\\.uid)\\]\\s*:\\s*\\[)[^\\]]*(\\])"
    )
    private val sensitivePropertyTextRegex = Regex(
        "(?i)((?:[a-z0-9_-]+\\.)*(?:imei\\d*|meid|imsi|iccid|android[_-]?id|cpuid|ufsid|fp\\.uid)\\s*[:=]\\s*)[^\\s,;|\\\"]+"
    )
    private val sensitivePropertyJsonRegex = Regex(
        "(?i)(\\\"(?:[^\\\"]*\\.)?(?:imei\\d*|meid|imsi|iccid|android[_-]?id|cpuid|ufsid|fp\\.uid)\\\"\\s*:\\s*\\\")[^\\\"]*(\\\")"
    )
    ...
    private fun sanitizeText(text: String, plan: ReplacementPlan): String {
        ...
        out = out
            ...
            .replace(sensitivePropertyBracketRegex, "$1$REDACTED_DEVICE_ID$2")
            .replace(sensitivePropertyTextRegex, "$1$REDACTED_DEVICE_ID")
            .replace(sensitivePropertyJsonRegex, "$1$REDACTED_DEVICE_ID$2")
