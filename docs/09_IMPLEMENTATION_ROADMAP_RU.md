...
| generic `openService` | **частично** | `AdbServiceCall` вызывает произвольный сервис и собирает вывод: общий дедлайн, потолок на приём, предел объёма, закрытие потока при любом из них — как ограниченный read-only probe в A2. Одного потока за раз хватает без фонового цикла; постоянный цикл придёт с интерактивным shell. Из production-кода пока не вызывается |
| `shell,v2` | **нет** | — |
| legacy fallback | **нет** | — |
