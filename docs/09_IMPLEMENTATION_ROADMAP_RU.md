...
| DATA OUT | **написано, гейт не пройден** | `FastbootLane.sendData` передаёт байты в открытую фазу, `FastbootDownload` ведёт `download:` целиком — 22 теста. Правила счёта байтов взяты из архивов, а не выведены: A2 `FastbootDataTransfer.transferSyncBulk` и Legacy `transferDownloadPayload` сходятся в трёх вещах — короткая запись хост → устройство законна и дописывается; запись «больше запрошенного» или «ноль и меньше» неоднозначна и **не повторяется** (`03` §3); ранний конец источника провал, а не успех. Объём объявляется **восемью** шестнадцатеричными цифрами (`%08x`, Legacy строки 923 и 1141) — четыре это длина ответа, другое поле другого направления. Блок 16 КиБ, как в `SYNC_BULK` у A2; нативный путь с 256 КиБ принадлежит `usb:native`, которого нет. Граница мутации разобрана: `download:` **не прошивает**, он наполняет буфер загрузки, и утверждать «ничего не изменилось» можно **только** при отказе до фазы данных — поле `untouched` в исходе и в журнале отвечает ровно на этот вопрос. `max-download-size` намеренно не проверяется: это диагностика, а не наша авторизация (`03` §2), и отказ по объёму принадлежит устройству. В production: `downloadGenerated` и две кнопки на экране; нагрузку порождает приложение, потому что выбор файла — artifact source из Phase 8 (тот же приём принят для sync `SEND`, §6.36). Гейт `07` §6.73 записан до прогона и ждёт его |
