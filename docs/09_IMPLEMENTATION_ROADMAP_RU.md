...
| Пункт | Статус | Где |
|---|---|---|
| Sync core | **готово** | `AdbSyncProtocol` и `AdbStreamBuffer` плюс `AdbSyncSession` со `STAT`, `RECV` и `SEND`. Read-only путь подтверждён на железе малым и 2 MiB файлом (`07` §6.29, §6.32). `SEND` написан, покрыт тестами и подключён к экрану, но на устройстве не проверялся — гейт `07` §6.34 |
...
| concurrent service dispatcher | **решение зафиксировано, код не начат** | `AdbStreamRouter` умеет маршрутизировать несколько logical streams, но production ownership пока даёт один активный reader consumer: цикл `reader.read()` крутит сам потребитель, поэтому потребитель может быть только один, и `AdbLinkController.busy()` гасит кнопки при живой оболочке. Замысел записан до кода в `docs/adr/0004_CONCURRENT_ADB_DISPATCHER_RU.md` (**PROPOSED**, D034): цикл переходит к `AdbConnection`, логические потоки получают ограниченные почтовые ящики, потребители переводятся по одному без второго production path. ADR принимается только после аппаратного прогона — правильность одновременных потоков не проверяется ни сборкой, ни подставным транспортом. Это архитектурный долг, а не искусственный product gate |
