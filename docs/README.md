...
- `0006_FASTBOOT_SINGLE_LANE_AND_THREE_STATE_ROLE_RU.md` — **PROPOSED** 2026-09-11: Fastboot не мультиплексирует, поэтому полоса обмена одна; роль устройства имеет три состояния, и `UNKNOWN` не сворачивается в «загрузчик». До аппаратного прогона (`07` §6.69) решение не принято.
