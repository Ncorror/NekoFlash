...
| `shell,v2` | **нет** | неинтерактивная оболочка работает через `shell:`; для `shell,v2` придётся расширить баннер хоста возможностями — без обоюдного объявления устройство его не включит |
| legacy fallback | **нет** | — |
