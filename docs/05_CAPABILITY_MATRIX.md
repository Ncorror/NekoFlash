...
| ADB | Legacy breadth, A2 framing tests, current multi-stream evidence + Phase 2 CNXN/AUTH slice | Minimal pure `:protocol:adb` handshake now exists for evidence only; one CNXN, AUTH as required, no services yet; future service engine keeps one physical reader and no artificial vendor whitelist |
